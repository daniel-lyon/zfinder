...
class Plot(object):
    ...
    def plot_peaks(y_axis, x_axis, plot_type):
        ...
        text_offset = max(y_axis)/20

        for i, snr, sc in zip(peaks, snrs, scales):
            plot_type.plot(x_axis[i], y_axis[i], marker='o', color='blue')
            plot_type.text(x_axis[i], y_axis[i]+0.4*text_offset, s=f'snr={snr}', color='blue')
            plot_type.text(x_axis[i], y_axis[i]+text_offset, s=f'scale={sc}', color='blue')
    ...
    def plot_chi2(self):
        ...
        self.d = count_decimals(self.obj.dz)
        ...
        for page, (chi2, colours) in enumerate(zip(all_chi2, AllColours)):
            ...
            for index, (c2, color) in enumerate(zip(chi2, colours)):
                ...
                axs[index].set_title(f'$\chi^2_r$ = {round(min(c2), 2)} @ z={round(lowest_redshift, self.d)}')
                axs[index].set_yscale('log')

            if self.obj.num_plots == 1:
                plt.savefig('Chi-squared Plot', dpi=300)
            else:
                plt.savefig(f'Chi-squared Plot ({page+1}).png', dpi=300)
            plt.show() # Show the plot
    ...
    def plot_flux(self):
        ...
        for page, (fluxes, chi2, params) in enumerate(zip(all_flux, all_chi2, all_params)):
            ...
            for index, (flux, c2, param) in enumerate(zip(fluxes, chi2, params)):
                ...
                axs[index].set_title(f'z={round(lowest_redshift, self.d)}')
                self.plot_peaks(flux, self.obj.x_axis_flux, axs[index])

            if self.obj.num_plots == 1:
                plt.savefig('Flux Best Fit', dpi=300)
            else:
                plt.savefig(f'Flux Best fit ({page+1}).png', dpi=300)
            plt.show()
