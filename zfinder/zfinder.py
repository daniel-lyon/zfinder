...
class zFinder(object):
    def __init__(self, image: str, right_ascension: list, declination: list, aperture_radius: float,
            bvalue: float, num_plots=1, minimum_point_distance=1.0, warnings=False, sigma=1):
        '''
    ...
    def export_csvs(self):
        lowest_index = np.argmin(self.all_chi2[0])

        # Export Best Fit Flux Parameters
        rows = zip(['amplitude', 'standard deviation'], self.all_params[0][lowest_index], self.all_uncertainties[0][lowest_index])
        with open('data.csv', 'w') as f:
            ...
            num_peaks = self.all_num_peaks[0][lowest_index]
            reduced_sigma = self.sigma**2 / (len(self.all_flux[0]) - 2*num_peaks - 1)
            uncert = Uncertainty(self.z, self.all_chi2[0], reduced_sigma)
            neg, pos = uncert.calc_uncert()

            # Export Uncertainty of the Flux data
            wr.writerow(['Flux Redshift and uncertainties'])
            wr.writerow(['-', 'z', '+'])
            wr.writerow([neg, self.z[np.argmin(self.all_chi2[0])], pos])
            wr.writerow([])

        # Write chi2 vs redshift
        rows = zip(self.z, self.all_chi2[0])
        with open('flux_chi2_data.csv', 'w') as f:
            wr = csv.writer(f)
            wr.writerow(['Redshift', 'Chi-squared'])
            for row in rows:
                wr.writerow(row)
        
        # Write flux vs frequency
        params = self.all_params[0][lowest_index]
        loc = self.ftransition/(1+self.all_lowest_z[0])
        fit = self.gaussf(self.x_axis_flux, a=params[0], s=params[1], x0=loc)
        rows = zip(self.x_axis_flux, self.all_flux[0], fit)
        with open('flux_best_fit.csv', 'w') as f:
            wr = csv.writer(f)
            wr.writerow(['Frequency', 'Flux', 'Fit'])
            for row in rows:
                wr.writerow(row)

    def zfind(self, ftransition, z_start=0, dz=0.01, z_end=10, plots=True, animations=False, files=True):
        ''' 
        ...
        ----------
        ftransition : `float`
            The first transition frequency of the target element/molecule/etc. Units are
            automatically taken from the .fits image wcs.
        ...
        for index, coord in enumerate(self.coordinates):
            ...
            for ddz in self.z:
                ...
                try:
                    params, covars = curve_fit(lambda x, a, s: self.gaussf(x, a, s, x0=loc), 
                        self.x_axis_flux, y_flux, bounds=[[0, (1/8)], [2*max(y_flux), (2/3)]], absolute_sigma=True) # best fit
                except RuntimeError:
                    chi2_array.append(max(chi2_array)) # if no returned parameters, set the chi-squared for this redshift to the maximum
                    num_peaks_array.append(0)
                    continue
                ...
                peaks = find_peaks(f_exp)
                num_peaks = len(peaks[0])

                chi2 = sum(((y_flux - f_exp) / uncert)**2)
                reduced_chi2 = chi2 / (len(y_flux) - 2*num_peaks - 1)
                ...
                chi2_array.append(reduced_chi2)
                param_array.append(params)
                uncert_array.append(perr)
            ...
            self.all_uncertainties.append(uncert_array)
            self.all_num_peaks.append(num_peaks_array)
...
def main():
    ...
    ftransition = 115.2712 # the first transition
